...
for LAYER in $LAYERS; do
  ...
  gsutil cp gs://$GCS_BUCKET/artifacts/$LAYER/terraform.$LAYER.$(git log --pretty=format:"%h" | sed -n 3p).plan $ARTIFACTS_DIR

  echo "terraform init  $LAYER"
  (cd "$LAYERS_DIR/$LAYER" && terraform init)

  echo "terraform apply $LAYER"
  (cd "$LAYERS_DIR/$LAYER" && terraform apply -input=false -no-color "$ARTIFACTS_DIR/terraform.$LAYER.$(git log --pretty=format:"%h" | sed -n 3p).plan")
